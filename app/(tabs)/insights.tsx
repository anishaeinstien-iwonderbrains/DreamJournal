...
function StatCard({ icon, label, value, sub, tint }: { icon: string; label: string; value: string; sub?: string; tint: string }) {
  return (
    <LiquidGlass radius={Radius.xl} tint={tint} glowColor={tint} intensity={85} style={styles.statCard}>
...
export default function InsightsScreen() {
  ...
  const stats = useMemo(() => {
    ...
    return { totalDreams, avgQuality, durationStr: h > 0 ? `${h}h ${m}m` : `${m}m` };
  }, [sessions]);
    ...
    return (
      <SafeAreaView style={styles.safe} edges={['top']}>
        ...
        <View style={styles.emptyState}>
          <LiquidGlass radius={Radius.full} tint={accent.primary} glowColor={accent.primary} intensity={85} style={styles.emptyIconWrap}>
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsGrid}>
          <StatCard icon="bedtime" label="Nights logged" value={String(sessions.length)} tint={accent.light} />
          <StatCard icon="cloud" label="Total dreams" value={String(stats?.totalDreams ?? 0)} tint="#40D8FF" />
          <StatCard icon="schedule" label="Avg sleep" value={stats?.durationStr ?? '--'} tint={Colors.accent} />
          <StatCard icon="star" label="Avg quality" value={stats ? stats.avgQuality.toFixed(1) : '--'} sub="/ 5.0" tint={Colors.accentSoft} />
        </View>
        ...
        {stats ? (
          <View style={styles.section}>
            ...
            <LiquidGlass radius={Radius.xl} tint={accent.primary} glowColor={accent.primary} intensity={85}>
        ...
        {tagFrequency.length > 0 ? (
          <View style={styles.section}>
            ...
            <LiquidGlass radius={Radius.xl} intensity={85}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.moodGrid}>
            {WAKE_MOODS.map((mood) => {
              ...
              return (
                <LiquidGlass key={mood.id} radius={Radius.xl} tint={accent.primary} glowColor={accent.primary} intensity={82} style={styles.moodItem}>
...
const styles = StyleSheet.create({
  ...
  blob: { position: 'absolute', width: 300, height: 300, borderRadius: 150, top: -80, right: -70 },
  ...
  statIconWrap: { width: 44, height: 44, borderRadius: Radius.lg, alignItems: 'center', justifyContent: 'center', marginBottom: Spacing.xs },
  ...
  sectionTitle: {
    fontSize: Typography.xs, fontWeight: Typography.bold, color: Colors.textSecondary,
    marginBottom: Spacing.md, textTransform: 'uppercase', letterSpacing: 1,
  },

  qualityContent: { padding: Spacing.xl, gap: Spacing.md },
  ...
  tagContent: { padding: Spacing.base, gap: Spacing.sm },
  tagRow: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md },
  tagDot: {
    width: 8, height: 8, borderRadius: 4,
    shadowOffset: { width: 0, height: 0 }, shadowOpacity: 0.9, shadowRadius: 5, elevation: 2,
  },
  ...
  barTrack: {
    ...
    borderRadius: Radius.full, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,0.10)',
  },
